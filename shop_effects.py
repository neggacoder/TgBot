...
# Всё, что нельзя ни купить, ни продать, ни подарить: медали за «наградить»
# и предметы за ачивки. И то и другое — свидетельство заслуги.
...
def shop_rows() -> list[tuple[str, str, int, str, str]]:
    """Всё, что нужно досеять в магазин: полезное, трофеи и предметы за ачивки.
    ...
    return ([i.as_shop_row() for i in EFFECT_ITEMS]
            + [i.as_shop_row() for i in REWARD_ITEMS]
