...
def test_сводка_считает_кулдауны_тем_же_способом_что_команды(мир):
    """Снаряжение мастерской режет кулдауны рыбалки, клада и смены — панель
    обязана спросить про КАЖДЫЙ из них тем же helper'ом, что и сами команды.

    Иначе получается худший вид расхождения: панель пишет «готово», человек
    пишет команду, и бот отвечает «слишком рано» — или наоборот, панель врёт
    про ожидание, которого уже нет.
    """
    import shop_effects as SE

    спрошено = []

    async def perk_cooldown(chat_id, user_id, base, perk):
        спрошено.append(perk)
        return base

    мир.setattr(bot_module, "_perk_cooldown", perk_cooldown, raising=False)
    _собрать()

    for perk in (SE.PERK_FARM_COOLDOWN, SE.PERK_FISH_COOLDOWN,
                 SE.PERK_TREASURE_COOLDOWN, SE.PERK_WORK_COOLDOWN):
        assert perk in спрошено, f"сводка не спросила {perk}"


def test_сводка_не_советует_выключенный_заработок(мир):
    """Рубильник заработка гасит команду молча — значит и предлагать её в
    «чем заняться» нельзя: человек нажмёт подсказку и не получит ничего."""
    было = set(bot_module.earnings_off)
    bot_module.earnings_off.clear()
    try:
        states, _ = _собрать()
        assert "farm" in _по_ключу(states), "ферма пропала из сводки и без рубильника"

        bot_module.earnings_off.update({"farm", "work"})
        states, _ = _собрать()
        по = _по_ключу(states)
        assert "farm" not in по, "сводка советует выключенную ферму"
        assert "profession" not in по, "сводка советует выключенную работу"
        assert "fishing" in по, "заодно погасили рыбалку — её никто не выключал"
    finally:
        bot_module.earnings_off.clear()
...
# ---------------------------------------------------------------------------
# Как панель написана: подписи — названия занятий, команды — то, что бот и
# правда понимает. Раньше подписью служила сама команда, и строка ожидания
# читалась как «🏢 бизнес собрать — 5 минут»: слева команда, справа срок.
# ---------------------------------------------------------------------------

def test_подписи_занятий_это_названия_а_не_команды():
    for a in activities.CATALOG:
        assert a.title[:1].isupper(), f"«{a.title}» — подпись с маленькой буквы"
        assert not set(a.title) & set("!/{}"), f"«{a.title}»: в подписи служебные знаки"


def test_каждая_команда_панели_настоящая():
    """Панель для того и нужна, чтобы команду скопировали тапом. Опечатка в
    ней — совет написать боту то, чего он не понимает."""
    чужие = [a.command for a in activities.CATALOG
             if bot_module.resolve_command_key(a.command) is None]
    assert not чужие, чужие


def test_в_ожидании_видно_название_и_срок():
    текст = _панель([_состояние("business", wait_note="копится"),
                     _состояние("robbery", left=timedelta(minutes=5))])
    assert "Доход бизнеса — копится" in текст, текст
    assert "Ограбление — 5 минут" in текст, текст
