...
def test_улов_и_смена_в_чат_не_уходят_кроме_награды_за_коллекцию():
    """Это личные занятия: в чат идут только достижения и редкое завершение
    коллекции, а не отчёт о каждом забросе."""
    исходник = inspect.getsource(api)
    отправки = исходник.count("send_message")
    assert отправки == 2, "в чат уходят только ачивка и завершённая коллекция"
    assert "send_message" in inspect.getsource(api._announce)
    assert "site_announcement" in inspect.getsource(api._check_collections)
