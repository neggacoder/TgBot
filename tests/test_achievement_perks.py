...
# Привилегии, которые СКЛАДЫВАЮТСЯ осознанно. Всё остальное дублироваться не
# должно: две вещи с одним переключателем — это либо мёртвая вторая, либо
# незамеченное удвоение процента.
_СКЛАДЫВАЮТСЯ = {SE.PERK_FARM_PLOTS}


def test_привилегии_не_дублируются():
    """Две вещи с одной привилегией складывались бы — это надо решать осознанно.

    Для грядок решено «складываются»: их и должно быть много, и каждая ачивка
    обязана давать свои — иначе вторая полученная не даст ничего.
    """
    perks = [i.perk for i in SE.ACHIEVEMENT_ITEMS
             if i.perk and i.perk not in _СКЛАДЫВАЮТСЯ]
