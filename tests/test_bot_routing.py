...
# ---------------------------------------------------------------------------
# +транскриб / -транскриб: только точные формы переключателя расшифровки
# ---------------------------------------------------------------------------

def test_транскриб_тумблер_доходит():
    assert "cmd_transcription_toggle" in handlers_for(message(text="+транскриб"))
    assert "cmd_transcription_toggle" in handlers_for(message(text="-транскриб"))


def test_текст_со_словом_транскриб_не_триггерит_переключатель():
    assert "cmd_transcription_toggle" not in handlers_for(message(text="я включил транскриб"))


# ---------------------------------------------------------------------------
# +сайт / -сайт: только точные команды доступа к кабинету участников
# ---------------------------------------------------------------------------

def test_сайт_тумблер_доходит():
    assert "cmd_site_toggle" in handlers_for(message(text="+сайт"))
    assert "cmd_site_toggle" in handlers_for(message(text="-сайт"))


def test_текст_со_словом_сайт_не_триггерит_тумблер():
    assert "cmd_site_toggle" not in handlers_for(message(text="сайт выключен"))


# ---------------------------------------------------------------------------
# «челопет» — РП-команда, а «пет» остаётся для личных животных
# ---------------------------------------------------------------------------

def test_челопет_запускает_запрос_человеку_питомцу():
    assert "cmd_human_pet_request" in handlers_for(message(text="челопет"))


def test_пет_продать_не_перехватывается_человеком_питомцем():
    taken = handlers_for(message(text="пет продать pes да"))
    assert "cmd_human_pet_request" not in taken
    assert "cmd_pet_sell" in taken


# ---------------------------------------------------------------------------
# Шуточное СВО: принимаются только заявленные команды
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("text", [
    "СВО отправить", "СВО вернуть", "СВО выкл", "СВО вкл", "СВО статус",
])
def test_команды_сво_доходят_до_обработчика(text):
    assert "cmd_svo" in handlers_for(message(text=text))


def test_лишний_текст_не_запускает_сво():
    assert "cmd_svo" not in handlers_for(message(text="СВО статус сейчас"))
...
# ---------------------------------------------------------------------------
# «-админ» — команда целиком, а не набор букв из неё
#
# В фильтре стояло `in ("-админ")` — без запятой это не кортеж, а СТРОКА, и
# проверка превращалась в поиск подстроки. Совпадало любое первое слово,
# которое встречается внутри «-админ»: «а», «и», «ад», «мин», «-». Живая речь
# («И чо это», «а почему его») уходила в снятие прав, и бот отвечал на неё
# «Укажите цель».
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("реплика", [
    "И чо это", "а почему его", "а", "и", "ад", "мин", "админ",
    "а что это такое", "ну и ладно",
])
def test_живая_речь_не_уходит_в_снятие_прав(реплика):
    assert "cmd_revoke_admin" not in handlers_for(message(реплика))


@pytest.mark.parametrize("команда", ["-админ", "-АДМИН", "-админ @vasya", "-админ 12345"])
def test_снятие_прав_по_своей_команде_работает(команда):
    assert "cmd_revoke_admin" in handlers_for(message(команда))


@pytest.mark.parametrize("команда,ожидается", [
    ("+завоз", "cmd_shop_restock_notify_toggle"),
    ("-завоз", "cmd_shop_restock_notify_toggle"),
])
def test_выключатель_сводки_о_завозе_доходит_до_обработчика(команда, ожидается):
    взяли = handlers_for(message(команда))
    assert взяли and взяли[0] == ожидается, f"{команда!r} -> {взяли}"
