...
def test_число_настроек_как_в_спеке():
    """31 — не магия: столько перечислено в спеках. Тест ловит потерянную
    строку при правках, а не проверяет арифметику.

    Было 25; +6 добавила настройка баланса заработка: пять множителей «кранов»
    и суточный лимит подработок."""
    assert len(chat_settings.SETTINGS) == 25 + len(chat_settings.INCOME_SOURCES) + 1


def test_у_каждого_крана_есть_своя_настройка():
    """INCOME_SOURCES — источник правды и для команды «доход», и для реестра.
    Строка, забытая в SETTINGS, означала бы источник, который команда обещает
    настроить, а панель о нём не знает."""
    ключи = {s.key for s in chat_settings.SETTINGS}
    потерянные = [w for k, w, _ in chat_settings.INCOME_SOURCES
                  if chat_settings.income_setting_key(k) not in ключи]
    assert not потерянные, "нет настройки для: " + ", ".join(потерянные)
