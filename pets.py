...
# Имя, которое хозяин даёт питомцу («пет назвать кот Барсик»). Обрезается по
# длине, а не отклоняется целиком: молча укоротить привычнее для человека,
# чем требовать переписать команду покороче.
NAME_MAX = 64
