...
class Activity:
    ...
    title — НАЗВАНИЕ занятия, с большой буквы и человеческими словами: оно
    стоит слева в строках «Ждём» и «Недоступно» («🏢 Доход бизнеса — копится
    500 i¢»). Раньше здесь лежала сама команда, и строка читалась как
    «🏢 бизнес собрать — 5 минут» — команда в роли подписи, да ещё и в
    повелительном наклонении рядом со сроком.

    command — то слово, которым занятие зовут. Оно печатается в <code>, чтобы
    в Telegram копировалось тапом; ради этого же оно пишется точно так, как
    его понимает бот, а не «красиво». Проверяется тестом: команда панели
    обязана опознаваться реестром.
    """
    key: str
    emoji: str
    title: str
...
CATALOG: tuple[Activity, ...] = (
    Activity("daily_bonus",  "🎁", "Ежедневный бонус", "бонус"),
    ...
    Activity("business",     "🏢", "Доход бизнеса",    "бизнес собрать"),
    Activity("robbery",      "🥷", "Ограбление",       "!ограбить"),
