...
async def api_member_pets(user: PanelUser = Depends(auth.require_member)):
    ...
    return {"ok": result.ok, "text": result.text,
            ...
            "catalog": await game_actions.pet_catalog_cards(chat_id, user.tg_user_id)}
...
# Продажа остаётся только в чате: уровень и опыт питомца исчезают безвозвратно,
# поэтому перед таким действием нужен привычный двухшаговый сценарий Telegram.
_DISABLED = {
...
# не остаться безымянной дырой. Покупка тоже читает ключ напрямую, поэтому
# ключ обязателен и для неё.
