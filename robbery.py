...
# Второй выход из-под надзора — время. Ста тысяч у попавшегося чаще всего нет
# и не будет (он же только что потерял 40% кошелька), поэтому платный откуп в
# одиночку означал не наказание, а вылет из механики навсегда. Две недели —
# срок, за который надзор успевает надоесть, но не превращается в вечный.
SURVEILLANCE_AUTO_PARDON = timedelta(days=14)
