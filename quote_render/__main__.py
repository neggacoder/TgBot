...
def _cases() -> list:
    ...
    return [
        ...
        # Имена рисуются не одним draw.text(), а через раскладку — иначе
        # эмодзи в имени (а они у половины чата) выходит пустым квадратом.
        # 🔥 и 👋🏽 взяты намеренно: оба лежат в assets/emoji, поэтому демо
        # рисуется одинаково и без сети (эмодзи вне набора докачалось бы с CDN
        # и осело файлом в assets/emoji — демо от такого лучше держать чистым).
        ("emoji_in_name", [QuoteMessage(
            user_id=10, name="Мария 🔥", text="эмодзи в имени автора",
            reply_name="Ержан 👋🏽", reply_text="и в имени того, кому отвечают 🔥",
            reply_chat_id=11,
        )]),
        # Длинное имя без пробелов: раскладка обязана обрезать его многоточием
        # по ширине плашки, а не вылезти за её край.
        ("long_names", [QuoteMessage(
            user_id=13, name="ОченьДлинноеИмяБезПробелов", text="проверка правого края",
            reply_name="ОченьДлинноеИмяКотороеТочноНеВлезаетВПлашкуРеплаяНикакВообще",
            reply_text="короткий текст", reply_chat_id=14,
        )]),
        ("group", [
